...
# Giữ nguyên ngữ nghĩa hai prompt từ bước 2 để so sánh công bằng.
SYSTEM_V1 = (
...
def run_rag(retriever, llm, prompt, question: str) -> dict:
    ...
    # RAGAS cần danh sách từng context riêng biệt.
...
def build_ragas_dataset(rag_results: list) -> EvaluationDataset:
    ...
    """
    # Ánh xạ kết quả RAG sang schema SingleTurnSample.
    samples = [
        SingleTurnSample(
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    # Đánh giá đủ bốn metrics theo rubric.
    # Gợi ý:
    #   result = evaluate(
    #       dataset,
    ...
    for key in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        values = np.asarray([v for v in result[key] if v is not None], dtype=float)
        scores[key] = float(np.nanmean(values)) if values.size else 0.0
...
def main():
    ...
    print(f"💾 Đã sao chép báo cáo evidence vào {evidence_report_path}")
