...
# Tên prompt duy nhất theo chủ sở hữu repository.
PROMPT_V1_NAME = "dothaiduong-rag-prompt-v1"
PROMPT_V2_NAME = "dothaiduong-rag-prompt-v2"
...
SYSTEM_V1 = (
    ...
    "Trả lời trực tiếp, ngắn gọn trong 2-4 câu. Nếu context không đủ thông tin, "
...
SYSTEM_V2 = (
    "Bạn là chuyên gia AI. Chỉ dựa trên context được cung cấp, hãy xác định các "
    "sự kiện liên quan rồi viết câu trả lời rõ ràng, chính xác và có tổ chức trong "
    "3-5 câu. Không bổ sung kiến thức ngoài context; nếu thiếu dữ kiện, hãy nói rõ."
    "\n\nContext:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    """
    # Push cả hai phiên bản độc lập để một lỗi không chặn phiên bản còn lại.
    try:
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    # Pull từ Hub, fallback local khi Hub tạm thời không khả dụng.
    try:
...
def get_prompt_version(request_id: str) -> str:
    ...
    """
    # MD5 chỉ dùng để chia bucket tất định, không dùng cho bảo mật.
...
# ── 6. Traced A/B Query ────────────────────────────────────────────────────
# Trace riêng cho mỗi A/B request.
...
def main():
    ...
    # Tạo LangSmith client và đồng bộ prompt versions.
    ...
    for i, question in enumerate(SAMPLE_QUESTIONS):
        ...
        if version_tag == "v1":
            ...
            v2_count += 1
        log_line = (
            f"[{i+1:02d}] [prompt-{version_tag}] Q: {question}\n"
            f"     A: {result['answer']}"
        )
    ...
    summary = (
        f"Routing: V1={v1_count} | V2={v2_count} | Tổng={len(SAMPLE_QUESTIONS)}"
    )
    log_path.write_text(
        "\n\n".join(routing_log + [summary]), encoding="utf-8"
    )
    print(f"💾 Đã lưu routing log vào {log_path}")
